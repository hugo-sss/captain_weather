    ...
    <div className="flex-1 min-h-0 flex flex-col">
      ...
      <div className="flex-1 min-h-0 grid lg:grid-cols-[1fr_400px]">
        <div className="relative min-h-[40vh] lg:min-h-[420px]">
          ...
          <label className="absolute top-2 right-2 z-[1000] rounded-md border border-border bg-bg-1/95 backdrop-blur-sm px-2.5 py-1.5 text-[11px] flex items-center gap-3 cursor-pointer shadow-[0_4px_16px_rgba(0,0,0,0.35)]"><span>OpenSeaMap <span className="text-text-3">crowdsourced, not official</span></span><Switch checked={prefs.show_openseamap} onCheckedChange={(v) => update({ show_openseamap: v })} aria-label="OpenSeaMap overlay" /></label>
          <div className="absolute bottom-2 left-2 z-[1000] rounded-md border border-border bg-bg-1/95 backdrop-blur-sm px-2.5 py-1.5 text-[11px] text-text-2 flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-accent" /> Click the map to drop a pin. Drag pins to move them.</div>
          {withEta.length >= 2 && (
            <div className="absolute bottom-2 right-2 z-[1000] rounded-md border border-border bg-bg-1/95 backdrop-blur-sm px-3 py-1.5 text-[12px] flex items-center gap-2 max-w-[60%]">
