...
export function OverlayToggles({ prefs, update, encStatus }: { prefs: DisplayPrefs; update: (p: Partial<DisplayPrefs>) => void; encStatus?: string | null }) {
  return (
    <div className="absolute top-2 right-2 z-[1000] rounded-md border border-border bg-bg-1/92 backdrop-blur-sm px-2.5 py-1.5 text-[11px] space-y-1.5 shadow-[0_4px_16px_rgba(0,0,0,0.35)]">
...
export function RiskLegend() {
  ...
  return (
    <div className="absolute bottom-2 left-2 z-[1000] rounded-md border border-border bg-bg-1/92 backdrop-blur-sm px-2 py-1 text-[10px] uppercase tracking-[0.06em] text-text-2 flex items-center gap-3">
