...
export function LegRow({ wp, c, maxWindKn, selected, onSelect, showComparison, utcOffsetMin, passageId }: LegRowProps) {
  ...
  return (
    <tr onClick={onSelect} className={cn('cursor-pointer', selected && 'is-selected', wp.arrived && 'text-text-2')} aria-selected={selected}>
      ...
      <Td className="num"><div className="leading-tight">{fmtUtc(eta)}</div><div className="text-[11px] text-text-3 leading-tight">{fmtLocal(eta, utcOffsetMin)}</div></Td>
      ...
      <Td className="num r">{gapA ? <Gap reason="no atmospheric data" w="w-8" /> : fmtNum(num(c?.gust_p90_kn), 0)}</Td>
      ...
      <Td className="num r" title={c?.ukc_basis ? `basis: ${c.ukc_basis}` : undefined}>{c?.ukc_estimate_m !== null && c?.ukc_estimate_m !== undefined ? <>{fmtNum(num(c.ukc_estimate_m), 1)}<Unit>m</Unit></> : <span className="text-text-3">—</span>}</Td>
      {showComparison && (
