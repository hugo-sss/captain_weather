...
type Props = { waypoints: WaypointRow[]; conditions: WaypointConditionsRow[]; maxWindKn: number | null; selectedId: string | null; onSelect: (id: string) => void; showComparison: boolean; utcOffsetMin: number | null };
...
export function LegTable({ waypoints, conditions, maxWindKn, selectedId, onSelect, showComparison, utcOffsetMin }: Props) {
      ...
      <table className="min-w-full text-sm">
        <thead className="border-b border-border">
          <tr>
            ...
            <Th>Models</Th><Th>Risk</Th><Th>Conf</Th>
          </tr>
        </thead>
        <tbody>
          {waypoints.map((wp) => (
            <LegRow key={wp.id} wp={wp} c={byWp.get(wp.id) ?? null} maxWindKn={maxWindKn} selected={selectedId === wp.id} onSelect={() => onSelect(wp.id)} showComparison={showComparison} utcOffsetMin={utcOffsetMin} />
          ))}
        </tbody>
      </table>
    </div>
