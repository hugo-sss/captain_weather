...
| `BRIEFING_BASE_URL` + `BRIEFING_API_KEY` + `BRIEFING_MODEL` | `generate-briefing` when `BRIEFING_PROVIDER=openai` | Point at Synthetic (`https://api.synthetic.new/openai/v1`), z.ai, OpenRouter, etc. Missing any of the three stores an unavailable row with the reason. |
